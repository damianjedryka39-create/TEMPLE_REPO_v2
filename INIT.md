...
Szablon leży w: `/root/GOFANS-NEOVERSE/TEMPLE_REPO/`
...
```bash
cp -r /root/GOFANS-NEOVERSE/TEMPLE_REPO/ <ŚCIEŻKA_NOWEGO_REPO>
cd <ŚCIEŻKA_NOWEGO_REPO>
rm -f INIT.md AGENTS1.md                     # INIT zbędny po użyciu; AGENTS1 to archiwum
```
...
Komenda (jedna, pełna — TIMESTAMP generowany dynamicznie):
```bash
...
Weryfikacja: `grep -rn "{{" .` musi zwrócić zero wyników. Jeśli coś zostało — dopisz do sed lub wypełnij ręcznie.
...
1. **NIE modyfikuj plików w `/root/GOFANS-NEOVERSE/TEMPLE_REPO/`** — to jest szablon-master. Pracujesz TYLKO w nowym repo.
