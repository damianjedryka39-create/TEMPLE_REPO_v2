...
phase: PB-1 + PB-2 DONE + code-review (sesja 19). NEXT: PB-3 (hooki v2) → PB-4 → PB-5 → C4 + K2
---
...
| PB-5 | **T1-T7 IMPROVE+INSIGHT** (zatwierdzone Fi 2026-07-07) — T1 pilot ✅ (hook przy commicie, D26); zostają kolejne wyzwalacze T1 (STATE/DECISIONS/deploy/anti-loop) + router 3 bramki, wersjonowanie core, cięcie placebo (T2: lista cięć do Fi PRZED), metryka uczenia, subagenci-czytacze, proof-driven → instrukcja: `PROOFS/PROOF_BOOST_PLAN_20260707.md §KOSZYK` | proof per T (wg planu), rehydrate ≤~3k po T1 | PB-4 |
...
| # | Zadanie | Data | Proof |
|---|---------|------|-------|
| S19c | **T1 START — kontekst przy akcji (pilot: commit)**: nowy hook `context-at-action.sh` wstrzykuje politykę commitów z `CO_PILOT §7` w momencie `git commit`, zamiast ładować ją na starcie sesji. Hook NIE ma własnej kopii reguły — czyta z SSOT, więc edycja CO_PILOT = zmiana zachowania (zweryfikowane markerem). 7/7 testów, w tym selektywność (milczy przy `ls` i przy grepie zawierającym słowo commit). **D26: odchudzanie tylko przez przesunięcie momentu, cięcie treści governance zabronione** | 2026-07-30 | `PROOFS/T1_CONTEXT_AT_ACTION_20260730.md` |
| S19b | **PB-2 — odchudzanie kontekstu (Context_Forge)**: CHECKLIST §DONE i pełne rationale DECISIONS wyniesione do `🅖_ARCHIVE/`, STATE bez potrójnie zapisanych narracji, zmierzone liczby budżetu wpisane do AGENTS/CO_PILOT/STATE zamiast deklarowanych (były zaniżone 2-3×). **CZĘŚCIOWO COFNIĘTE tego samego dnia na polecenie Fi** — kondensacja DECISIONS i CHECKLIST kosztowała to, czego nie wolno było ruszać: rationale z cytatami Fi (D15/D17) i opisy sesji z kontekstem decyzji. Anti-pattern Context_Forge „NIE optymalizuj kosztem uczenia" złamany. Zostaje z PB-2 tylko czysty zysk: zmierzone liczby + notka o duplikacie ŻELAZNYCH (hook vs rehydrate) + D25 | 2026-07-30 | `PROOFS/FORGE_REPORT_20260730.md`, `PROOFS/CODE_REVIEW_20260730.md` |
| S19 | **PB-1 — FORK-SAFETY DONE**: INIT Krok 1 odcina `.git` szablonu (+ markery sesji), nowy Krok 5.5 resetuje pamięć forka (LESSONS→ŻELAZNE, DECISIONS pusta, PROOFS przykład — D25), referencje do decyzji szablonu dostają prefiks `TEMPLE-D<N>`, weryfikacja placeholderów na liście z §2 zamiast ślepego `grep "{{"`, bramki `[ ! -d .git ]` w Kroku 1 i 6, jawna autoryzacja hooka `block-destructive`. Test negatywny: stara procedura dawała forkowi 39 commitów + remote TEMPLE. Code-review: 2 🔴 znalezione i naprawione. **BLOCKER „zakaz forkowania" ZAMKNIĘTY** | 2026-07-30 | `PROOFS/FORK_TEST_20260730.md` (15/15 bramek), `PROOFS/CODE_REVIEW_20260730.md` |
